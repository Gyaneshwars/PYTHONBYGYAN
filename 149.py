...
def DCS1(historicalData, filingMetadata, extractedData, parameters,data1):
    ...
    #and (gaap_exc=="" or filingMetadata["metadata"]["GAAP"] not in gaap_exc)\
        ...
        # Replace NaN values with an empty string in the "capitalStructureCleanedDescription" column
        df['capitalStructureCleanedDescription'].fillna('', inplace=True)

        # Filter rows where capitalStructureCleanedDescription doesn't contain alphabets (excluding NULL values)
        df_filtered = df[~df['capitalStructureCleanedDescription'].astype(str).str.contains(r'[a-zA-Z]') & (df['capitalStructureCleanedDescription'] != '') ]
